...
if (!dir.exists("Results/Original")) {
  dir.create("Results/Original", recursive = TRUE)
}
...
write.csv(d, "Data/database_original.csv", row.names = FALSE)
...
print(table2_output, file="Results/Original/table2_original.txt", type="latex")
write.csv(round(synth.tables$tab.pred,1), "Results/Original/table2_original.csv")
...
print(table1_output, file="Results/Original/table1_original.txt", type="latex")
write.csv(tab1, "Results/Original/table1_original.csv")

#### Figure 1: Trends in Per-Capita GDP: West Germany vs. Rest of the OECD Sample
Text.height <- 23000
Cex.set <- .8
pdf(file = "Results/Original/ger_vs_oecd_original.pdf", width = 5.5, height = 5.0, family = "Times", pointsize = 12)
...
pdf(file = "Results/Original/ger_vs_synthger_original.pdf", width = 5.5, height = 5.0, family = "Times", pointsize = 12)
par(mar=c(5.1,4.1,1,2.1))  # Match margins from main analysis script
...
pdf(file = "Results/Original/ger_vs_synthger_gaps_original.pdf", width = 5.5, height = 5.0, family = "Times", pointsize = 12)
par(mar=c(5.1,4.1,1,2.1))  # Match margins from main analysis script
...
write.csv(gap_data, "Results/Original/gap_data_original.csv", row.names = FALSE)
...
Cex.set <- 1
pdf(file = "Results/Original/placebo1975_original.pdf", width = 5.5, height = 5.0, family = "Times", pointsize = 12)
par(mar=c(5.1,4.1,1,2.1))  # Match margins from main analysis script
...
pdf("Results/Original/ratio_post_to_preperiod_rmse_original.pdf", width = 7, height = 6.5, family = "Times", pointsize = 12)
par(mar=c(5.1,4.1,1,2.1))  # Match margins from main analysis script
...
Text.height <- 23000
Cex.set <- .8
pdf(file = "Results/Original/jackknife_original.pdf", width = 5.5, height = 5.0, family = "Times", pointsize = 12)
par(mar=c(5.1,4.1,1,2.1))  # Match margins from main analysis script
plot(1960:2003,dataprep.out$Y1plot,
     type="l",ylim=c(0,33000),col="black",lty="solid",
     ylab ="per-capita GDP (PPP, 2002 USD)",
...
if (!require(gtools, quietly = TRUE)) {
  ...
  # Save Table 3
  capture.output(table3, file = "Results/Original/table3_original.txt")
  save(table3, file = "Results/Original/table3_original.RData")
  ...
  # Save Table 4
  write.csv(table4, "Results/Original/table4_original.csv")
  
  ## Figure 7: Per-Capita GDP Gaps Between West Germany and Sparse Synthetic Controls
  Text.height <- 23000
  Cex.set <- .8
  
  pdf(file = "Results/Original/sparse_synthetic_controls_original.pdf", width = 11, height = 8.0, family = "Times", pointsize = 12)
  par(mfrow=c(2,2), mar=c(5.1,4.1,3,2.1))  # Match margins with space for subplot titles
  for(pp in 4:1){
...
} # End of conditional block for gtools and kernlab

cat("Analysis completed. Results saved in 'Results/Original' folder:\n")
cat("- Figures saved as PDF files with matched dimensions\n")
cat("- Tables saved as TXT and CSV files\n")
cat("- Database saved as database_original.csv in Data folder\n")